...
-- web sp's --
DELIMITER $$
CREATE PROCEDURE spSelectExercise (IN in_id INT, IN userKey NVARCHAR(255))
BEGIN
        SELECT * FROM (SELECT  CONVERT(AES_DECRYPT(difficulty,userKey) USING utf8) AS difficulty ,CONVERT(AES_DECRYPT(name,userKey) USING utf8) AS name, from PECI_PROJ.Exercises WHERE id =in_id ); 
END $$
DELIMITER ;
